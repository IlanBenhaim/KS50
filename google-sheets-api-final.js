...
const SPREADSHEET_ID = '1C8vxKa8O3PIrM4zg9XIEOCT6FzM7uMP__KOyCi4Jdmw';
 ...
 */
function doGet(e) {
  // Configuration CORS pour permettre l'accès depuis n'importe quel domaine
  ...
  try {
    ...
    return output.setContent(JSON.stringify(stats));
  } catch (error) {
...
function getContributionStats() {
  try {
    ...
    Logger.log('Erreur dans getContributionStats: ' + error.toString());
    return {
      error: true,
      message: error.toString(),
      totalEUR: 0,
